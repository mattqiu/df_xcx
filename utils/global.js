...
class Global{
  ...
  goBind() {
    this.WxService.navigateTo('/pages/login/bind');
  }
  //重新登录
  refreshLogin(){
  ...
  link(url){
    ...
    var switchTabUrl = ['/pages/index/index','/pages/xinfang/list','/pages/my/index'];
    var isSwitch = false;
    var _ = this._;
    _.each(switchTabUrl,(v,k)=> {
      if(url.indexOf(v) != -1){
        if(k == 1){
          ...
          wx.setStorageSync('from_xinfang_params',obj);
        }
        this.WxService.switchTab(v);
        isSwitch = true;
      }
    })
    if(!isSwitch){
  ...
  getNoteList(){
    return new Promise(resolve=>{
      var app = getApp();
      if(app.globalData.nodeList){
        resolve(app.globalData.nodeList);
      }else{
        app.nodeListCallback.push(function(obj) {
          resolve(obj);
        });
      }
    })
  ...
  isLogin(){
    var app = getApp();
    return (app.globalData.user && app.globalData.user['is_true'] == 1) ? 1 : 0;
  }
  //获得配置
  getConfig(){
    return new Promise(resolve=>{
